...
with st.sidebar:
    ...
    st.download_button("💾 作業データを保存(JSON)", data=save_json, file_name="matsumatsu_lesson_plan.json", mime="application/json")
...
# 質問リストの定義
QUESTIONS = [
    {"key": "subject", "label": "教科名", "type": "text", "prompt": "①【教科名】を入力してください。（例: 国語、数学、社会）"},
    {"key": "teacher", "label": "授業者名", "type": "text", "prompt": "②【授業者名】を入力してください。（例: 山田 太郎）"},
    {"key": "unit_title", "label": "単元（題材）名", "type": "text", "prompt": "③【単元（題材）名】を入力してください。"},
    ...
    {"key": "tsukamu", "label": "三松メソッド ① つかむ", "type": "textarea", "prompt": "⑨ 三松メソッド【① つかむ】（課題設定・導入など）の内容を入力してください。"},
    {"key": "kangaeru", "label": "三松メソッド ② 考える", "type": "textarea", "prompt": "⑩ 三松メソッド【② 考える】（個人思考・探究など）の内容を入力してください。"},
    {"key": "manabi", "label": "三松メソッド ③ 学び合う", "type": "textarea", "prompt": "⑪ 三松メソッド【③ 学び合う】（対話・協働など）の内容を入力してください。"},
    {"key": "matomeru", "label": "三松メソッド ④ まとめる・振り返る", "type": "textarea", "prompt": "⑫ 三松メソッド【④ まとめる・振り返る】（まとめ・リフレクションなど）の内容を入力してください。"},
]

# 左右2列レイアウト（左：入力フォーム ／ 右：元の様式のリアルタイム表プレビュー）
...
with col_left:
    ...
    if step < len(QUESTIONS):
        ...
        with st.form(key=f"q_form_{step}"):
            if q["type"] == "text":
                val = st.text_input(q["label"], value=st.session_state.data.get(q["key"], ""))
            elif q["type"] == "textarea":
                val = st.text_area(q["label"], value=st.session_state.data.get(q["key"], ""))
            elif q["type"] == "number":
                val = st.number_input(q["label"], min_value=1, max_value=20, value=int(st.session_state.data.get(q["key"], 5)))
    ...
    # 時間ごとの詳細計画フェーズ（思考スキル選択機能つき）
        ...
        if h_idx < tot:
            st.info(f"**【第 {h_idx+1} / {tot} 時間目】** の計画を入力してください")
            curr_h = hours[h_idx]
            
            with st.form(key=f"h_form_{h_idx}"):
                c_val = st.text_area("学習内容・主な活動", value=curr_h.get("content", ""))
                
                # 19の思考スキル多角選択
                all_skills = [s for cat in THINKING_SKILLS.values() for s in cat]
                s_val = st.multiselect("使用する思考スキル（複数選択可）", options=all_skills, default=curr_h.get("skills", []))
            ...
            st.success("🎉 すべての項目の入力が完了しました！右側の完成シートをご確認ください。")
...
with col_right:
    ...
    for idx, h in enumerate(d.get("hours", [])):
        skills_tags = "".join([f'<span style="background-color:#e1f5fe; color:#0277bd; padding:2px 6px; margin:2px; border-radius:4px; display:inline-block; font-size:11px;">{s}</span>' for s in h.get("skills", [])])
        if not skills_tags:
    ...
    # 元の様式そのままの美しいHTML表構造を生成
    html_preview = f"""
    <div style="border: 2px solid #2b5797; padding: 15px; border-radius: 8px; background-color: #ffffff; font-family: 'Helvetica Neue', Arial, sans-serif; font-size: 13px; color: #333; line-height: 1.5;">
